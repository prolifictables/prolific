...
async function fetchOrderStatus(orderId: string): Promise<any> {
  // SSR-safe: apiGet on Render wake detect will throw, withFallbackNull swallows
  // → returns null instead of { error } or raw HTML leak.
  // Skeleton shown; client re-fetches through OrderStatusClient → overlay shown.
...
export default async function WebsiteOrderStatusPage({
  ...
  // SSR safety: on any server-side error (especially Render cold-start waking),
  // render a branded skeleton. Client-side hydration will re-fetch via the
  // guarded apiGet wrapper → shows our custom overlay if backend is waking.
  if (!data) {
    return (
